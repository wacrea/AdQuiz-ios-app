...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [Flurry startSession:@"H6SH2MSMG4R4QXVBNQRB"];
    ...
    if(![[NSUserDefaults standardUserDefaults] boolForKey:@"AlreadyRan"] ) {
        ...
        NSMutableDictionary *tempCurrentPage = [[NSMutableDictionary alloc] init];
        [tempCurrentPage setValue:[NSNumber numberWithInt:0] forKey:@"number"];
        
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *documentsDirectory = [paths objectAtIndex:0];
        NSString *path = [documentsDirectory stringByAppendingPathComponent:@"currentPage.plist"];
        
        [tempCurrentPage writeToFile:path atomically: YES];
    }
    
    // Init main view controller
    AQLevelsViewController * levels = [[AQLevelsViewController alloc] init];
    navigationController = [[UINavigationController alloc] initWithRootViewController:levels];
